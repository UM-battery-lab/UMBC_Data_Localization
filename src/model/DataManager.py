...
class DataManager:
    ...
    def process_cell(self, cell_name, numFiles = 1000):
        ...
        """
        #TODO: Use more elegant way to get grandparent path, maybe change the way we store data
        tr_path = self.dataFilter.filter_trs(device_name_substring=cell_name)[0]
        print(tr_path)
        cell_path = self.__get_grandparent_path(tr_path)
        ...
        # Filepaths for trs
        trs_neware_path = self.dataFilter.filter_trs(device_name_substring=cell_name, tags=['neware_xls_4000'])
        trs_arbin_path = self.dataFilter.filter_trs(device_name_substring=cell_name, tags=['arbin'])
        trs_biologic_path = self.dataFilter.filter_trs(device_name_substring=cell_name, tags=['biologic'])
        trs_vdf_path = self.dataFilter.filter_trs(device_name_substring=cell_name, tags=['vdf'])        
        # Load trs for cycler data
        trs_neware = self.dataProcessor.sort_tests(self.dataIO.load_trs(trs_neware_path))
        trs_arbin = self.dataProcessor.sort_tests(self.dataIO.load_trs(trs_arbin_path))
        trs_biologic = self.dataProcessor.sort_tests(self.dataIO.load_trs(trs_biologic_path))
        trs_cycler = self.dataProcessor.sort_tests(trs_neware + trs_arbin + trs_biologic)
        trs_vdf = self.dataProcessor.sort_tests(self.dataIO.load_trs(trs_vdf_path))
        # Process data
    ...
    def __get_grandparent_path(self, path):
